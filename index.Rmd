...
```{r}
top_users_pictured %>%
  select(-c(id, user_avatar, profile_url)) %>%
  reactable(
    .,
    pagination = TRUE,
    ...
    columns = list(
      username = colDef(
        ...
        cell = function(value, index) {
          profile <-
            str_glue(
              '<img class="img-fluid img-circle mr-2" src="{top_users_pictured[index, "user_avatar"]}" alt="{top_users_pictured[index, "id"]}">'
            )
      ...
      posts_read = colDef(
        ...
        defaultSortOrder = "desc",
        cell = function(value) {
          build_bar_col(value, top_users_pictured$posts_read, "#3fc1c9")
        },
      ),
      post_count = colDef(
        name = "Post Count",
        defaultSortOrder = "desc",
        cell = function(value) {
          build_bar_col(value, top_users_pictured$post_count, "#fc5185")
        },
      ),
      days_visited = colDef(
        name = "Days Visited",
        defaultSortOrder = "desc",
        cell = function(value) {
          build_bar_col(value, top_users_pictured$days_visited, "#3fc1c9")
        },
      ),
      topics_entered = colDef(
        ...
        defaultSortOrder = "desc",
        cell = function(value) {
          build_bar_col(value, top_users_pictured$topics_entered, "#fc5185")
        },
      )
    ),
    compact = TRUE,
